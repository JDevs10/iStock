            ...
            try{
                for(let x = 0; x < data_.length; x++){
                    await db.transaction(async (tx) => {
                        await tx.executeSql("INSERT INTO " + TABLE_NAME + " ("+COLUMN_ID+", "+COLUMN_REF+", "+COLUMN_LABEL+", "+COLUMN_LIEU+", "+COLUMN_FK_PARENT+", "+COLUMN_COUNTRY+", "+COLUMN_COUNTRY_ID+", "+COLUMN_COUNTRY_CODE+", "+COLUMN_STATUT+") VALUES ("+data_[x].id+", '"+data_[x].ref+"', '"+data_[x].label.replace(/'/g, "''")+"', '"+data_[x].lieu.replace(/'/g, "''")+"', "+(data_[x].fk_parent == null ? null : "'"+data_[x].fk_parent+"'")+", '"+data_[x].country+"', '"+data_[x].country_id+"', '"+data_[x].country_code+"', '"+data_[x].statut+"')", []);
                    });
                }
                return await resolve(true);
            } catch(error){
