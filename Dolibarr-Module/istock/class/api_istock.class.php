...
class IStockApi extends DolibarrApi
{
	...
	public function login($login, $password, $entity='', $reset=0) 
	{
		...
		$sql_channel = "SELECT rowid FROM llx_c_input_reason WHERE code LIKE '%ISTOCK%' AND label = 'iStock'";
		$result_channel = $this->db->query($sql_channel);
		$iStock_channel = null;
		
		if ($result_channel)
        {
            if($this->db->num_rows($result_channel) > 0){
				while($row__ = $this->db->fetch_array($result_channel)){
					$iStock_channel = $row__["rowid"];
				}
			}
        }
	...
	/*################################################################################################################################*/
	/*#############################################  Gestion Api General #############################################################*/
	#region Gestion Api General
	
	/**
     * Get iStock channel
     *	
	 * @return  obj                               Ojb of iStock channel
     *
     * @url	GET get/info/channel
     */
	 function getIstockChannel(){
		if (! DolibarrApiAccess::$user->rights->istock->configuration->read) {
			throw new RestException(401);
		}
		
		$sql = "SELECT rowid FROM llx_c_input_reason WHERE code LIKE '%ISTOCK%' AND label = 'iStock'";
		$result = $this->db->query($sql);
		$iStock_channel = -99;
		...
		if ($result)
        {
            ...
            if($this->db->num_rows($result) > 0){
				...
				while($row__ = $this->db->fetch_array($result)){
				...
				throw new RestException(204, 'iStock channel not found: '.$this->db->lasterror());
			}
        }
        else {
            throw new RestException(503, 'Error when retrieve iStock channel : '.$this->db->lasterror());
        }
	 }
	 
	
	/**
     * Get total number of objects from orders before download
     *	
	 * @param string	       $sortfield	        Sort field
     * @param string	       $sortorder	        Sort order
     * @param int		       $limit		        Limit for list
	...
	public function sync_v2($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $thirdparty_ids = '', $sqlfilters = ''){
		...
		if($obj_ret != null && count($obj_ret)>0){
			...
			for($index=0; $index<count($obj_ret); $index++){
				...
				($this->checkArrayDouble($tmp_order, $obj->id) ? null : $tmp_order[] = $obj->id );
				
				// get and store products of order
				for($_index_=0; $_index_<count($obj->lines); $_index_++){
					$tmp_orders_lines[] = $obj->lines[$_index_]->id;
					($this->checkArrayDouble($tmp_products_ids, $obj->lines[$_index_]->fk_product) ? null : $tmp_products_ids[] = $obj->lines[$_index_]->fk_product );
				}
				
				// get and store contact of order
				$tmp_res_oc = $this->orderContactById($obj->id);
				if($tmp_res_oc != null && $tmp_res_oc != "null"){
					($this->checkArrayDouble($orders_contacts, $tmp_res_oc->rowid) ? null : $orders_contacts[] = $tmp_res_oc->rowid );
				}
				
				// get and store client of order
				$tmp_res = $this->clientById($obj->socid);
				if($tmp_res != null){
					($this->checkArrayDouble($tmp_clients, $tmp_res->rowid) ? null : $tmp_clients[] = $tmp_res->rowid );
				}
				
				// get and store user of order
				$tmp_res = $this->userById($obj->user_author_id);
				if($tmp_res != null){
					($this->checkArrayDouble($tmp_users, $tmp_res->id) ? null : $tmp_users[] = $tmp_res->id );
				}
				if($tmp_res_oc != null && $tmp_res_oc != "null"){
					($this->checkArrayDouble($tmp_users, $tmp_res_oc->fk_socpeople) ? null : $tmp_users[] = $tmp_res_oc->fk_socpeople );
				}
			}
		...
		if($tmp_products_ids != null && count($tmp_products_ids)>0){
			...
			for($index=0; $index<count($tmp_products_ids); $index++){ 
				...
				if($tmp_res != null){
					($this->checkArrayDouble($tmp_products, $tmp_res->id) ? null : $tmp_products[] = $tmp_res->id );
				}
			}
		...
		if($tmp_order_ids != null && count($tmp_order_ids)>0){
			...
			for($index=0; $index<count($tmp_order_ids); $index++){ 
				...
				if($tmp_res != null){
					($this->checkArrayDouble($tmp_shipments, $tmp_res->id) ? null : $tmp_shipments[] = $tmp_res->id );
				}
				if($tmp_res->lines != null){
					for($_index_=0; $_index_<count($tmp_res->lines); $_index_++){
						$tmp_shipments_lines[] = $tmp_res->lines[$_index_]->id;
					}
				}
		...
		return array(
			"orders"=>$tmp_order,
			"orders_lines"=>$tmp_orders_lines,
			"orders_contacts"=>$orders_contacts,
			"clients"=>$tmp_clients,
			"users"=>$tmp_users,
			"products"=>$tmp_products,
			"shipments"=>$tmp_shipments,
			"shipments_lines"=>$tmp_shipments_lines,
			"Total_objs"=> ((count($tmp_order) + count($tmp_orders_lines) + count($orders_contacts) + count($tmp_clients) + count($tmp_users) + count($tmp_products) + count($tmp_shipments) + count($tmp_shipments_lines))),
			"Total_objs_msg"=> "TT orders: ".count($tmp_order)." | TT orders lines: ".count($tmp_orders_lines)." | TT order contacts: ".count($orders_contacts)." | TT clients: ".count($tmp_clients)." | TT users: ".count($tmp_users)." | TT products: ".count($tmp_products)." | TT shipments: ".count($tmp_shipments)." | TT shipments lines: ".count($tmp_shipments_lines)." | TT all: ".(count($tmp_order) + count($tmp_orders_lines) + count($orders_contacts) + count($tmp_clients) + count($tmp_users) + count($tmp_products) + count($tmp_shipments) + count($tmp_shipments_lines))
		);
	...
	/*
	private function getLatestLot_DLC_DLUO_BatchOfProduct($nb_days){
		$obj_ret = array();
	...
	*/
     ...
     * @return array
     *
     * @url	GET get/client/{rowid}
    */
    public function clientById($rowid)
    {
     ...
     * @return array
     *
     * @url	GET get/user/{rowid}
    */
    public function userById($rowid)
    {
     ...
     */
	public function orderIndex($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $sqlfilters = ''){
		
		$obj_ret = array();
		
		$sql = "SELECT * FROM llx_element_contact as t ";
		// Add sql filters
        if ($sqlfilters) {
            if (!DolibarrApi::_checkFilters($sqlfilters)) {
                throw new RestException(503, 'Error when validating parameter sqlfilters '.$sqlfilters);
